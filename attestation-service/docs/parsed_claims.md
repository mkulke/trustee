...
The claim inherit the fields from the TDX claim with and additional `tpm` hierachy in which the TEE's PCR values are stored:
...
The claim inherit the fields from the SEV-SNP claim with and additional `tpm` hierachy in which the TEE's PCR values are stored:
